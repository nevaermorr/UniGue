...
public class General {
  	...
  	static public void VarDump(object obj) {
  		Debug.Log(General.GenerateVarDumpString(obj));
  	}
  	
  	// generate string for VarDump();
  	// modified version of http://ruuddottech.blogspot.com/2009/07/php-vardump-method-for-c.html
	static public string GenerateVarDumpString(object obj, int recursion=0) {
			        ...
			        if (value != null) {
			            ...
			            try {
			                if (!(value is ICollection)) {
								// Call GenerateVarDumpString() again to list child properties
			                    // This throws an exception if the current property value
			                    // is of an unsupported type (eg. it has not properties)
								result.Append(GenerateVarDumpString(value, recursion + 1));
			                }
			                else {
			                    ...
			                    foreach (object element in ((ICollection)value)) {
									...
									result.Append(GenerateVarDumpString(element, recursion + 2));
			                        elementCount++;
			                    }
